...
class LocationOnMap {
  ...
  Future<DocumentReference> _addToDatabase() async {
    print("Location added");
    GeoFirePoint point = geo.point(latitude: position.latitude, longitude: position.longitude);
    return firestore.collection('Locations').add({'routeId': routeId, 'timestamp': position.timestamp, 'position': point.data, 'speed': position.speed});
  }
}
...
class Route {
  ...
  Section addSection() {
    Section section = Section(id: , routeId: this.id, from: );
    this.sections.add(section);
    return section;
  }

  endSection() {
    section._addToDatabase();
  }
...
class _MapPageState extends State<MapPage> {
  ...
  _addNewRating(Section section, Position to) {
    setState(() {
      ...
      actionsBuilder: (context, stars) {
        return [
          FlatButton(
            ...
            onPressed: () {
              currentRoute.endSection(stars: stars);
  ...
  void _listenToLocationChange(Route route) {
    ...
    _locationChangeSubscription = positionStream.listen((Position newPosition) async {
      LocationOnMap current = LocationOnMap(routeId: route.id, timestamp: DateTime.now(), position: newPosition, speed: newPosition.speed);
      current._addToDatabase();
  ...
  void _listenToStop(Route route) {
    Position last = route.from;
    var delta = 10.0;
    Duration timeSinceDepart;
    ...
    _stopSubscription = positionStream.listen(
      (Position newPosition) async {
        ...
        if ((newPosition.speed < 1 || delta < 1) && (timeSinceDepart > Duration(seconds: 10)) && (timeSinceLastRating > Duration(seconds: 20))) {
          print("STOPPED!!");
          _ratingDialog(route, newPosition);
                              ...
                              RaisedButton(
                                onPressed: () async {
                                  ...
                                  setState(() {
                                    ...
                                    currentRoute = Route(from: _startPosition, to: _destinationPosition);
                              ...
                              RaisedButton(
                                onPressed: () async {
                                  ...
                                  setState(() {
                                    ...
                                    _listenToLocationChange(currentRoute);
                                    _listenToStop(currentRoute);
                                  });
                                },
